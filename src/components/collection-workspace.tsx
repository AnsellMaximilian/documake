...
export function CollectionWorkspace({ collectionId, initialValues = {}, returnRecordId }: { collectionId: string; initialValues?: Record<string, unknown>; returnRecordId?: string }) {
  ...
  const [fieldForm, setFieldForm] = useState({ key: "", keyTouched: false, label: "", type: "text" as FieldType, required: false, configText: "", targetCollectionId: "" }); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [deletingRecordId, setDeletingRecordId] = useState<string | null>(null); const [summaries, setSummaries] = useState<Record<string, { sum: number | null; average: number | null }>>({});
  ...
  async function removeRecord(row: RecordRow) {
    if (!confirm(`Delete this ${schema?.name ?? "collection"} record? Relations and source links will be removed. Original documents remain in Inbox.`)) return;
    setDeletingRecordId(row.id); setError("");
    try { await clientApi(`/api/records/${row.id}`, { method: "DELETE" }); await load(); }
    catch (e) { setError(e instanceof Error ? e.message : "Could not delete record."); }
  ...
  return <div>
    ...
    <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      {tab === "records" && <Button size="sm" onClick={() => setShowRecordForm(true)} className="shrink-0 self-start sm:self-auto"><Plus className="size-4" />New record</Button>}
    </div>
    {tab === "records" ? <div className="mt-5 space-y-5">
      ...
      <Modal open={showRecordForm} onClose={() => { if (!busy) setShowRecordForm(false); }} title={`New ${schema.name} record`} description="Add structured values and keep the source evidence close.">
        <form onSubmit={createRecord}>
          <div className="space-y-6 px-5 py-6 sm:px-6">
            {returnRecordId && <div className="rounded-xl bg-accent-soft px-4 py-3 text-sm text-accent-strong">Creating a related {schema.name} record. The relation is already selected.</div>}
            {!schema.fields.length && <div className="flex items-start gap-3 rounded-xl border border-amber-300/70 bg-amber-50 px-4 py-3 text-amber-950"><AlertTriangle className="mt-0.5 size-5 shrink-0 text-amber-700" /><div><p className="text-sm font-bold">No structured fields yet</p><p className="mt-1 text-xs leading-5 text-amber-800">This will create a source-only record. You can add fields from Schema and fill it in later.</p></div></div>}
            {schema.fields.length > 0 && <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{schema.fields.map((field) => <FieldInput key={field.id} field={field} value={values[field.key]} related={relations[field.id] ?? []} onChange={(value) => setValues((current) => ({ ...current, [field.key]: value }))} />)}</div>}
            <SourceDocumentPicker documents={documents} selectedIds={sourceDocumentIds} onChange={setSourceDocumentIds} />
          </div>
          <div className="flex flex-col-reverse gap-3 border-t border-ink/10 bg-surface-muted/50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <label className="flex items-center gap-2 text-sm text-muted"><input type="checkbox" checked={draft} onChange={(e) => setDraft(e.target.checked)} className="size-4 accent-[var(--accent)]" />Save as draft for review</label>
            <div className="flex gap-2"><Button type="button" variant="secondary" onClick={() => setShowRecordForm(false)} disabled={busy}>Cancel</Button><Button disabled={busy}><Check className="size-4" />{schema.fields.length ? "Save record" : "Save source-only record"}</Button></div>
          </div>
        </form>
      </Modal>
      ...
      <Card className="overflow-hidden"><div className="flex items-center gap-2 border-b px-4 py-3"><Search className="size-4 text-muted" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter records…" className="w-full bg-transparent text-sm outline-none" /></div><div className="overflow-x-auto"><table className="w-full min-w-2xl text-left text-sm"><thead className="bg-surface-muted text-xs text-muted"><tr>{schema.fields.map((field) => <th key={field.id} className="px-4 py-3 font-medium">{field.label}</th>)}<th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y">{visible.map((row) => <tr key={row.id} className="hover:bg-surface-muted/60">{schema.fields.map((field) => <td key={field.id} className="max-w-64 truncate px-4 py-3">{formatValue(field, row.values[field.key], relations[field.id] ?? [])}</td>)}<td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${row.status === "draft" ? "bg-amber-100 text-amber-800" : "bg-accent-soft text-accent-strong"}`}>{row.status}</span></td><td className="px-4 py-3"><div className="flex items-center justify-end gap-1"><Link href={`/records/${row.id}`} aria-label="Open record" title="Open record" className="grid size-8 place-items-center rounded-lg text-muted transition hover:bg-white hover:text-ink"><ChevronRight className="size-4" /></Link><button type="button" onClick={() => removeRecord(row)} disabled={deletingRecordId === row.id} aria-label="Delete record" title="Delete record" className="grid size-8 place-items-center rounded-lg text-muted transition hover:bg-red-50 hover:text-red-600 disabled:opacity-40"><Trash2 className="size-4" /></button></div></td></tr>)}</tbody></table>{!visible.length && <div className="grid min-h-44 place-items-center text-center text-sm text-muted">No matching records.</div>}</div></Card>
